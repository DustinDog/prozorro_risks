...
async def process_tender(tender):
    ...
    try:
        tender_region = tender["procuringEntity"]["address"]["region"]
        tender_region = tender_region.strip().lower()
        if tender_region.startswith("закар"):
            tender["procuringEntity"]["address"]["region"] = "Закарпатська область"
